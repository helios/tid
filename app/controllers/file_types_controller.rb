class FileTypesController < ApplicationController
  ...
  # GET /file_types.json
  def index
     @search = FileType.search do
    fulltext params[:query_file_types]
  end
  @file_types = @search.results
  respond_to do |format|
    format.html
    format.json {    render :json => @file_types.map(&:attributes) }
  end
end
  # GET /file_types/1
  ...
  private
    ...
    def file_type_params
      params.require(:file_type).permit(:label, :date)
    end
end
